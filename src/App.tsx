...
const Modal = ({ config, onClose, onGoogleSignIn, onPaymentSuccess, purchasedBatches, onAction, onEmailAuth, onResetPassword, trialEndsAt, user }: { config: any, onClose: () => void, onGoogleSignIn: () => void, onPaymentSuccess: (batch: string) => void, purchasedBatches: string[], onAction: (t: string, ty: string) => void, onEmailAuth: (mode: 'login' | 'register', e: string, p: string) => void, onResetPassword: (e: string) => void, trialEndsAt: number, user: any }) => {
  const isWider = config.type === 'practice' && user && trialEndsAt > Date.now();
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      >
        <button onClick={onClose} className={`absolute p-1 z-50 ${isWider ? 'right-2 sm:right-4 top-2 sm:top-4 text-white hover:text-gray-200' : 'right-3 sm:right-4 top-3 sm:top-4 text-gray-400 hover:text-gray-600'}`}>
          <X className="w-6 h-6" />
        </button>
        {!isWider && <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4 pr-6">{config.title}</h3>}
